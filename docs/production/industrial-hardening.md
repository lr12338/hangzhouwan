...
门禁要求 Sidecar、A/B RTSP、A/B RTMP、输出不低于 9fps、推理不低于
4fps、滑动一小时重连不超过 2 次，且事件存储未进入保护状态。

每路健康同时公开 `lifecycle`、退出码/失败阶段、RTSP/RTMP
重连尝试与失败数、输入到达间隔 P95/P99。首次取帧只有 15 秒启动期，
60 秒仍无帧即失败；任一必需流失败时总体为 `FAILED`。
...
Video、三次严格健康确认，总上限 180 秒。若已开始重启但验证失败，
系统会停止 Video/Business 并写入
`/data/hangzhouwan/monitor/maintenance-recovery.json`；独立 timer 每 30
分钟检查上游、`/data` 和根盘门禁，门禁通过才完整重建，直至恢复健康。
Supervisor 在该状态存在时不并发恢复。

根盘低于 2GB 告警，低于 1.5GB 禁止发布、维护重启和自动恢复；已运行
视频只告警不停流。诊断只允许写入 `/data`，保留 7 天且总量不超过
500MB。
...
频繁服务重启掩盖。候选版本先连续观察 24 小时，再进入 72 小时签收。
签收要求 A/B 平均输出均不低于 9fps、推理不低于 4fps、端到端 P95
低于 500ms、每路重连尝试不高于 2 次/小时，且 RSS、TPU 内存、FD、
线程和根盘均无持续增长。签收前必须结合摄像机响应时间、网卡丢包、
输入到达间隔 P95/P99、队列丢帧、RTSP 错误与相机固件完成排查。
