...
| 项 | 值 |
|---|---|
| 候选 Release | `vpu-reconnect-health-4745180`（已构建+verify 8/8，未激活） |
| 生产 current | `202607221953-3dfcf4e`（旧版，有泄漏，仍在 VPU 错误风暴） |
| 生产 previous | `202607221953-3dfcf4e`（**与 current 相同，无独立回滚目标**） |
| 激活授权 | 未授予（不得切换 current/重启生产/故障注入） |

| 门禁 | 状态 | 说明 |
|---|---|---|
| G0 启动 | 未执行（需停生产） | 候选 verify+离线 smoke 通过；端到端启动须维护窗口停产后执行 |
| G1 真实RTSP 100轮 | **NOT_PASSED（硬阻断）** | 无真实 RTSP 断流源；`forced_reconnect_run.sh` 生产运行时拒绝执行 |
| G2 RTMP恢复 | 未执行 | 需 RTMP 服务器+维护窗口 |
| G3 A/B隔离 | 未执行 | 需维护窗口 dual 100 轮 |
| G4 VPU熔断+健康 | 部分验证 | 健康逻辑单测 17 项通过；板端熔断+健康状态需维护窗口验证 |
| G5 双路业务 | 未执行 | 需维护窗口 |
| G6 长稳 | 未执行 | 需维护窗口 2-4h（建议 24h） |
| G7 激活/回滚演练 | 未执行 | 需维护窗口；脚本语义已代码审查+临时目录测试通过 |

**结论：G1 未通过 -> 整体 NO-GO，不得激活候选。**
